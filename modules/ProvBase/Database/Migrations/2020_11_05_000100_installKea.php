...
class InstallKea extends BaseMigration
{
    ...
    public function up()
    {
        ...
        system("sudo -u postgres /usr/pgsql-16/bin/psql -c \"ALTER DATABASE kea OWNER TO $user;\"");

        system("/usr/sbin/kea-admin db-init pgsql -u $user -p $psw -n kea");

        system("sudo -u postgres /usr/pgsql-16/bin/psql -d kea -c \"
        ...
        system("for tbl in `sudo -u postgres /usr/pgsql-16/bin/psql -qAt -c \"select tablename from pg_tables where schemaname = 'public';\" kea`;
            do sudo -u postgres /usr/pgsql-16/bin/psql -d kea -c \"alter table ".'$tbl'.' owner to '.$user.'"; done');
