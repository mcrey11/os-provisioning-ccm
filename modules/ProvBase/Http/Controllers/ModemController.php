...
class ModemController extends \BaseController
{
    ...
    public function api_setLanMode($ver, $id)
    {
        if ($ver !== '0') {
            return response()->v0ApiReply(['messages' => ['errors' => ["Version $ver not supported"]]]);
        }
        ...
        if (! in_array(request('model'), ['TG3442S', 'TG3442SP'])) {
        ...
        try {
            switch (request('model')) {
                ...
                case 'TG3442SP':
                    // bridge -> router NAT (2)
                    snmpset($fqdn, $config->rw_community, '1.3.6.1.4.1.17270.50.2.3.2.1.1.32', 'i', 2);
                    break;
            }
        } catch (\Exception $e) {
            return response()->v0ApiReply(['messages' => ['errors' => [$e->getMessage()]]]);
    ...
    public function prepare_input($data)
    {
        ...
        $data['lat'] = data_get($data, 'lat') ? str_replace(',', '.', $data['lat']) : null;
        $data['lng'] = data_get($data, 'lng') ? str_replace(',', '.', $data['lng']) : null;
